...
const RPC_URL =
  ...
  process.env.POLYGON_AMOY_RPC_URL || "https://1rpc.io/matic";

if (!PRIVATE_KEY) {
  console.warn(
    "PRIVATE_KEY environment variable not set. Web3 functionality will be limited."
  );
}
...
export const walletClient: any = account
  ? createWalletClient({
      account,
      chain: polygon,
      transport: http(RPC_URL),
