 ...
 */
export async function executeApproval(step: ApprovalStep): Promise<ExecutionResult> {
  try {
    ...
    if (result.success) {
      console.log(`✅ Approval successful: ${step.token.address} → ${step.spender.address}`);
    } else {
